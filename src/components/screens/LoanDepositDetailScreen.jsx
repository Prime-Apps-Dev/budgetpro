    ...
    return (
      <div className="p-6 pb-24 bg-gray-50 min-h-screen dark:bg-gray-900">
        <div className="flex items-center mb-8">
          ...
          <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-200">Ошибка</h2>
        </div>
        <p className="text-center text-gray-500 dark:text-gray-400">Элемент не найден.</p>
      </div>
    );
  }

  // Находим самый актуальный объект из состояния
  const currentItem = (item.type === 'loan' ? loans.find(l => l.id === item.id) : deposits.find(d => d.id === item.id));

  // Если вдруг элемент не найден в обновленном списке, возвращаемся назад
  if (!currentItem) {
...
export default LoanDepositDetailScreen;